...
bool Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen) {
    ...
    if (SDL_Init(SDL_INIT_EVERYTHING) == 0) {
        ...
        // Creamos el renderer: el encargado de hablar con la tarjeta gráfica
...
void Game::handleEvents() {
    ...
    // Actualizamos el estado del teclado después de procesar los eventos
    InputHandler::Update();
}

void Game::update() {
    player->Update(); // El objeto recalcula sus rectángulos internos
    enemy->Update();
}
